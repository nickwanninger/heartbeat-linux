...
extern struct hb_rollforward rollforward_table[];
extern struct hb_rollforward rollback_table[];
long interval_us = 100;

volatile int done = 0;
void alarm_handler() { done = 1; }

__thread volatile uint64_t my_prev_handler_timestamp = 0;
...
__thread volatile uint64_t my_nb_handler_calls = 0;
...
int main(int argc, char **argv) {
  ...
  printf("cfg:%s, nproc:%lu, result:%lu, handler calls:%lu, array_block_len: "
         "%lu, array_len: %lu, interval_us: %lu\n",
         cfg, nproc, result, handler_calls, array_block_len, array_len,
         interval_us);
